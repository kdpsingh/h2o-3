...
def feature_frequencies():
    ...
    ff_gbm = gbm.predict_feature_frequencies(prostate_train)
    
    assert ff_gbm.shape == (len(features), prostate_train.nrow)
    
    drf = H2ORandomForestEstimator(ntrees=5)
    drf.train(x=features, y="CAPSULE", training_frame=prostate_train)
    ff_drf = drf.predict_feature_frequencies(prostate_train)

    assert ff_drf.shape == (len(features), prostate_train.nrow)

    iforest = H2OIsolationForestEstimator(ntrees=5)
    iforest.train(x=features, training_frame=prostate_train)
    ff_iforest = drf.predict_feature_frequencies(prostate_train)

    assert ff_iforest.shape == (len(features), prostate_train.nrow)
