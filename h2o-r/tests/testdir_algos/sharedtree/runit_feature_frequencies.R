...
test.predict_feature_frequencies <- function() {
    ...
    ff_gbm <- h2o.predict_feature_frequencies(gbm, prostate_hex)
    ...
    ff_drf <- h2o.predict_feature_frequencies(drf, prostate_hex)
    ...
    ff_iforest <- h2o.predict_feature_frequencies(iforest, prostate_hex)

    expect_equal(nrow(ff_iforest), nrow(prostate_hex))
    expect_equal(ncol(ff_iforest), ncol(prostate_hex))
}

doTest("Shared Tree: predict_feature_frequencies", test.predict_feature_frequencies)
